...
def main(config):
    ...
    for epoch in range(1, config.n_epochs+1):
        ...
        for i, (profiles, parameters) in enumerate(train_loader):
            ...
            gen_mode = config.gen_parameter_mode
            ...
            latent_vector = Variable(FloatTensor(np.random.normal(0, 1, (batch_size, config.latent_dim))))
            p = cgan_fake_parameters_gen_input(config.n_parameters, batch_size, global_parameters, mode=gen_mode)
            ...
            gen_loss = adversarial_loss(validity, all_ones)
            ...
            d_real_loss = adversarial_loss(validity_real, all_ones)

            # Loss for fake profiles
            validity_fake = discriminator(gen_profiles.detach(), gen_parameters)
            d_fake_loss = adversarial_loss(validity_fake, all_zeros)
            ...
            # print(
            #     "[Epoch %d/%d] [Batch %d/%d] [D loss: %e] [G loss: %e]"
            #     % (epoch, config.n_epochs, i+1, len(train_loader), d_loss.item(), gen_loss.item())
            # )
        ...
        #  Be mindful when each should be in train and eval mode
        #
