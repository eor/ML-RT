...
# -----------------------------------------------------------------
#  OLD implementation of Conditional variational auto-encoder
# -----------------------------------------------------------------
class RTcvae(nn.Module):

    # TODO: Review this and possibly delete
    # -----------------------------------------------------------------
    #  idea here:
    #  same  as VAE but input and latent vector are concatenated,
    #  or conditioned, with the parameter vector
    # -----------------------------------------------------------------

    def __init__(self, sizeInputLayer=1500, nParams=5):
        super(RTcvae, self).__init__()

        nLatent = 20

        # encoder
        self.fc1 = nn.Linear(sizeInputLayer + nParams, 1000)
        self.fc2 = nn.Linear(1000, 300)
        ...
        self.fc5 = nn.Linear(300, 1000)
        self.fc6 = nn.Linear(1000, sizeInputLayer)
    ...
    def decode(self, z):
        # TODO for RTcvae:  z plus p concat
        h4 = F.leaky_relu(self.fc4(z))
        h5 = F.leaky_relu(self.fc5(h4))
        return self.fc6(h5)
    ...
    def forward(self, x, y):
        ...
        pp = torch.cat((prof, para),
                       1)  # dimension 1 is profile length and parameters, dimension 0 is the batch (size)

        mu, logVar = self.encode(pp)  # 1505

        z = self.reparameterize(mu, logVar)

        zp = torch.cat((z, para), 1)  # condition latent vector with parameters

        return self.decode(zp), mu, logVar
